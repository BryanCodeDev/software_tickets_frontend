...
const PDAs = () => {
  ...
  const calculateStats = () => {
    ...
    const disponible = pdas.filter(i => i.status === 'disponible').length;
    const enUso = pdas.filter(i => i.status === 'en uso').length;
    const mantenimiento = pdas.filter(i => i.status === 'mantenimiento').length;
    const fueraServicio = pdas.filter(i => i.status === 'fuera de servicio').length;
    
    const warrantyExpiring = pdas.filter(item => {
    ...
    const totalValue = pdas.reduce((sum, item) => sum + (item.cost || 0), 0);
  ...
  const handleEdit = (item) => {
    ...
    setFormData({
      ...
      serial: item.serial,
      tipo_conectividad: item.tipo_conectividad || '',
  ...
  const handleDelete = async (id) => {
    showConfirmDialog('¿Estás seguro de que deseas eliminar esta PDA? Esta acción no se puede deshacer.', async () => {
      try {
  ...
  const exportToExcel = () => {
    const headers = ['IT', 'Propiedad', 'Responsable', 'Área', 'Marca', 'Modelo', 'Serial', 'Conectividad', 'Aplicaciones', 'Sistema Operativo', 'Estado', 'Garantía', 'Compra', 'Mantenimiento', 'Costo'];
    const rows = filteredPdas.map(item => [
      item.it,
      item.propiedad,
      item.responsable,
      item.area,
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'PDAs');
    ...
    <div className={conditionalClasses({
              ...
              <div className="flex items-center gap-3 lg:gap-4 mb-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className={conditionalClasses({
                    ...
                    Gestión integral de PDAs corporativas · 2025
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className={conditionalClasses({
            ...
            <h3 className={conditionalClasses({
              ...
              {searchTerm || filterStatus !== 'all' || filterArea !== 'all'
                ? 'No se encontraron PDAs'
                : 'No hay PDAs disponibles'}
            </h3>
            <p className={conditionalClasses({
              ...
              {searchTerm || filterStatus !== 'all' || filterArea !== 'all'
                ...
                : 'Comienza agregando una nueva PDA al inventario'}
            </p>
                        ...
                        <div key={item.id} className={conditionalClasses({
                          ...
                          <div className="flex items-start justify-between mb-3">
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-xs text-blue-600 font-medium truncate">{item.propiedad}</p>
                            </div>
                            <div className="flex gap-2">
                              <button
                                onClick={() => handleHistory(item)}
                          ...
                          <div className={conditionalClasses({
                            ...
                            <div>
                              ...
                              <p className="truncate">{item.tipo_conectividad || '-'}</p>
                            </div>
                            <div>
                              <span className="font-medium">SO:</span>
                              <p className="truncate">{item.sistema_operativo || '-'}</p>
                            </div>
                            <div className="col-span-2">
                              <span className="font-medium">Serial:</span>
                              <p className={conditionalClasses({
                                light: 'font-mono bg-gray-50 px-2 py-1 rounded text-xs break-all',
                                dark: 'font-mono bg-gray-600 px-2 py-1 rounded text-xs break-all'
                              })}>{item.serial}</p>
                            </div>
                  ...
                  <table className="w-full">
                    <thead className="bg-linear-to-r from-[#662d91] to-[#8e4dbf] text-white">
                      <tr>
                        ...
                        <th className="px-4 py-4 text-left text-xs font-bold uppercase tracking-wider">Conectividad</th>
                        <th className="px-4 py-4 text-left text-xs font-bold uppercase tracking-wider">Aplicaciones</th>
                          ...
                          <tr key={item.id} className={conditionalClasses({
                            ...
                            <td className="px-4 py-4">
                              <span className={conditionalClasses({
                                ...
                                dark: 'text-xs font-mono bg-gray-600 px-2 py-1 rounded'
                              })}>{item.serial}</span>
                            </td>
                            <td className="px-4 py-4">
                              <div className={conditionalClasses({
                                light: 'text-sm text-gray-900',
                                dark: 'text-sm text-gray-300'
                              })}>{item.tipo_conectividad || '-'}</div>
                            </td>
                            <td className="px-4 py-4">
                              <div className={conditionalClasses({
                                light: 'text-sm text-gray-900 truncate max-w-xs',
                                dark: 'text-sm text-gray-300 truncate max-w-xs'
                              })}>{item.aplicaciones || '-'}</div>
                            </td>
                            <td className="px-4 py-4">
              ...
              <div className={conditionalClasses({
                ...
                <h3 className={conditionalClasses({
                  ...
                  <div className="w-6 h-6 lg:w-8 lg:h-8 bg-purple-100 rounded-lg flex items-center justify-center">
                    <FaCog className="w-3 h-3 lg:w-4 lg:h-4 text-purple-600" />
                  </div>
                  Especificaciones Técnicas
                </h3>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={conditionalClasses({
                        ...
                        dark: 'w-full px-4 py-3 border-2 border-gray-600 rounded-xl focus:ring-2 focus:ring-[#8e4dbf] focus:border-transparent outline-none transition-all font-mono text-sm lg:text-base bg-gray-700 text-white'
                      })}
                      required
                    />
                  </div>
                ...
                >
                  {formLoading ? (
                    ...
                    <>
                      ...
                      {editingItem ? 'Actualizar PDA' : 'Crear PDA'}
                    </>
                  )}
                </button>
              </div>
      ...
      {showHistoryModal && historyItem && (
        <ActaEntregaHistoryModal
          ...
          title={`Historial - PDA ${historyItem.it}`}
          item={historyItem}
          loading={historyLoading}
          apiCall={() => pdaInventoryAPI.getHistory(historyItem.id)}
          moduleName="pda"
        />
      )}
