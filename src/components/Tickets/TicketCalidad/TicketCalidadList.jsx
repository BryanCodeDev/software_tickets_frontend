...
import TicketCalidadCard from './TicketCalidadCard';

const TicketCalidadList = React.memo(({
  filteredTickets,
  tickets,
  viewMode,
  setViewMode,
  ...
  // Función helper para obtener colores de estado (mismo que TicketList)
  ...
  const resultsText = useMemo(() => {
    const hasFilters = searchTerm || filterStatus !== 'all' || filterPriority !== 'all' || titleFilter;
    ...
    return {
      count: totalFiltered,
      total: total,
      text: hasFilters ? 'No se encontraron tickets de calidad' : 'No hay tickets de calidad disponibles',
      description: hasFilters ? 'Intenta ajustar los filtros de búsqueda' : 'Comienza creando un nuevo ticket de calidad para reportar problemas documentales'
    };
  }, [filteredTickets.length, tickets.length, searchTerm, filterStatus, filterPriority, titleFilter]);
...
TicketCalidadList.displayName = 'TicketCalidadList';

export default TicketCalidadList;